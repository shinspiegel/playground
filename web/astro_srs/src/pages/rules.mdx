...
3. **Roll the Dice**
   ...
   <ExampleBox closedText="Circumstantial Modifiers">

   Usually if not specified, the _Critical Threshold_ is **12**, and the _Fumble Threshold_ is **2**. That means, the maximum possible value for the dice or minimum value for the dice.
...
2. **Taking a turn** (and deciding the actions)
   ...
   Once every combatant have a their initiative value, from the _highest_ to the _lowest_ will take an actions for the round. A single major and minor action will be taken on each round. That means that the fastest combatant will take action first.
...
An Full Action, this is a complex and demanding action. Requiring the full focus of the character attention for the turn. A Major Action, this is a complex action, but does not demand full attention. And a Minor Action, this is a simple task, that does not require focus.
...
- **Minor Action List**
  ...
  - *Interact with the environment*: Most of the interaction with the environment use this action. Such as opening doors, shifting levers, pushing buttons.
...
<ExampleBox closedText="Improvising vs Preparation">
